...
func (cs *CommentsStorage) GetAll(onlyApproved bool, articleIdStr string) ([]*models.CommentRead, error) {
	rows, err := cs.db.Query("SELECT comment_id, article, content_ru, content_en, date_create, is_approved, author FROM comments WHERE article = $1 AND is_approved = $2", articleIdStr, onlyApproved)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (cs *CommentsStorage) Approve(commentApprove *models.CommentApprove, contentField string) error {
	_, err := cs.db.Exec("UPDATE comments SET is_approved = true, "+contentField+" = $1 WHERE comment_id = $2 AND is_approved = false", commentApprove.ContentPart, commentApprove.Id)
	if err != nil {
		return err
	}
	return nil
